...
class Video(local_nmf_base.VideoWrapper):
    ...
    def __init__(self,
        ...
        self.mov = local_nmf_factor.TensorFactor(video_shape[0],
                                scratch=False,
                                device=self.device,
                                dtype=torch.float32)
...
class LowRankVideo(local_nmf_base.VideoWrapper):
...
class RegionMetadata(local_nmf_base.FactorCollection):
    ...
    def __init__(self, max_num_components, region_shape, device='cuda'):
        ...
        self.support = local_nmf_factor.TensorFactor((max_num_components, region_shape[0]),
                                    device=device,
                                    dtype=torch.uint8)
        self.distance = local_nmf_factor.TensorFactor((max_num_components, region_shape[0]),
                                     device=device,
                                     dtype=torch.float32)
        self.labels = local_nmf_factor.TensorFactor((max_num_components,),
                                   device=device,
                                   dtype=torch.long)
