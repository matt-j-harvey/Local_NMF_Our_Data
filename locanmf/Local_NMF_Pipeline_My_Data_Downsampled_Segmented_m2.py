...
def run_local_nmf_pipeline(base_directory, early_cutoff=3000):
    ...
    # Load Atlas
    atlas = np.load(r"/home/matthew/Documents/M2_Segmentation/Downsampled_Churchland_Atlas.npy")
    ...
    print("Initial V", np.shape(v))

    # Moving Average
    v = moving_average(np.transpose(v))
    v = np.transpose(v)
    print("V shape post smoothing", np.shape(v))
